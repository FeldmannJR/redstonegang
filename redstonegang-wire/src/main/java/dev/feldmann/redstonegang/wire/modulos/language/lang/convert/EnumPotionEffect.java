...
public enum EnumPotionEffect {
    ...
    public static String getUnlocalizedName(ItemStack potion) {
        PotionType type = getPotionType(potion);
        EnumPotionEffect effect = get(type);
        return effect != null ? effect.unlocalizedName : type.name();
    }

    private static PotionType getPotionType(ItemStack potion) {
        PotionMeta meta = (PotionMeta) potion.getItemMeta();
        Potion po = Potion.fromItemStack(potion);
        if (po != null) {
            return po.getType();
        }
        return null;
    ...
    private static boolean isSplash(ItemStack it) {
        return Potion.fromItemStack(it).isSplash();
    }
