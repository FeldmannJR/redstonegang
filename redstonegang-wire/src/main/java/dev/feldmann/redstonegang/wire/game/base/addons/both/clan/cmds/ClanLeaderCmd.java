...
public abstract class ClanLeaderCmd extends ClanSubCmd {
    public ClanLeaderCmd(ClanAddon addon, String name, String desc, Argument... args) {
        super(addon, name, desc, args);
        setExecutePerm(cs -> {
            User pl = RedstoneGangSpigot.getPlayer(((Player) cs).getUniqueId());
            return getAddon().getCache().hasClan(pl.getId()) && getAddon().getCache().getMember(pl.getId()).getRole() >= ClanRole.LEADER;
    ...
    protected Function<CommandSender, Boolean> preProcessCommandAsync() {
        return (sender) -> {
            ...
            if (member.getClanTag() != null && member.getRole() >= ClanRole.LEADER) {
    ...
    public boolean canExecute(CommandSender cs) {
        ...
        User pl = RedstoneGangSpigot.getPlayer(((Player) cs).getUniqueId());
        if (!getAddon().getCache().hasClan(pl.getId()) || getAddon().getCache().getMember(pl.getId()).getRole() < ClanRole.LEADER) {
            C.error(cs, "Você precisa ser lider de um clan para usar este comando!");
            return false;
        }
        return true;
    }
