...
public class Add extends ConsoleCmd {
    ...
    public Add() {
        super("add", "adiciona cash para um jogador", PLAYER, QUANTIDADE);
    }

    @Override
    public void command(ConsoleCommandSender player, Arguments args) {

        args.get(PLAYER).addCash(args.get(QUANTIDADE));
        C.info(player, "Adicionado %% cash para %%!", args.get(QUANTIDADE), args.get(PLAYER));
    }
}
